...
namespace Algorytm_PD_dla_wiTi
{
    class wiTi
    {
        ...
        public static int CountPunishment(List<Task> tasks, int taskCount)  // Liczy karę dla obecnego porzadku w liscie
        {
            ...
            // Główna pętla iteruje 2^ilość tasków -1 razy dlatego zaczynamy od 1, tyle mamy podproblemów do zbadania
            for (int i = 1; i < ((int)Math.Pow(2,taskCount)); i++) 
            {
                ...
                for(int j = 0;j< binValue.Length; j++)               // petla w kórej sprawdzamy nasze podproblemy 
                {
                    ...
                    reversedString = "";
                    foreach(var bin in tmp)                       // tu odwracamy wyjaśnienie cały ten foreach 
                    {                                             // jako całość bym traktował w sumie razem nawewt z tmp
                        reversedString += bin;     // odwrócenie wartści z binvalue żebyśmy czytali zadania 
                                                   // od lewej np dla zadania 2 bitowo wynosito "10"                                              
                    ...
                    for(int k = 0; k < reveredStringArr.Length; k++)  // w tej petli sprawdzamy na któym miejscu stoi 1 czyli 
                    {                                                 // oznaczenie ze konkretny task jest rozpatrywany
                    ...
                    if(binValue != confBitBin)             // sprawdzamy czy nasze zadanie jest równe aktualnemu robimy to zeby 
                    {                                       // tylko raz liczyć karę dla konkretnego podproblemu
                        ...
                        maxValue = Math.Max(count - tasks[j].deadline, 0) *  // liczymy naszą karę dla danego podproblemu
                            tasks[j].punishment + resultList[confBit];       // wartośc maksymalna z (czasy wykonania tasków-dedline 
                                                                             // danego taska zero w przypadku gdy nie  ma spóźnienia 
                                                                             // tego taska)* kara + kara poprzedniego podproblemu
